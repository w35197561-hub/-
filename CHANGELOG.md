...
新增 TextareaComponent（原生 textarea，readonly），用 useComponentStyle 保证样式响应式。Select/Textarea 均改用原生元素替代 el-_，解决样式无法穿透 el-_ 包装层的问题。精简各组件 styleSetters 为精准暴露（仅保留视觉有效的属性）。coding.md 新增"canvas 禁用 el-\*"规则及 styleSetters 精准暴露指南。
