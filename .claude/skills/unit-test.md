...
**c. 组件业务逻辑测试（有逻辑时必须生成，不可省略）**

默认 props / undo/redo 是最低要求，**不是全部**。只要组件有非平凡的业务逻辑，reviewer 必须主动识别并生成对应的逻辑测试，不能等用户指出。

判断是否需要逻辑测试的标准：组件有计算行为，且计算结果影响用户可见的输出。

常见场景及对应测试重点：

| 逻辑类型 | 必须测试的内容 |
|----------|----------------|
| 排序 | 每个排序态（升序/降序/无）下数据顺序是否正确 |
| 分页 | 第 1 页 / 翻页后数据切片是否正确；pageSize=0 时全量显示 |
| 排序 + 分页组合 | 必须先全量排序再切片，不能先切片再排序 |
| 过滤 | 过滤条件变化后，结果集是否正确 |
| 级联 / 树形 | 展开/折叠状态、父子关系是否正确 |

生成逻辑测试时，直接抽 composable 或纯函数测试，**不要以"无法 mount 组件"为由放弃逻辑验证**。
...
> **重要限制**：composable / 纯函数测试只能验证逻辑本身，**无法验证模板是否正确使用了逻辑的输出**。
> 例如：composable 返回的 `pagedRows` 顺序正确，但模板里用了错误的变量（如 `editRows[rIdx]` 而非 `row[col.field]`），单元测试完全看不到这个 bug。
>
> **凡是组件有"逻辑影响渲染内容"的场景（排序、过滤、分页等），必须配合 E2E 测试验证实际渲染结果**，单元测试不能替代 E2E。
