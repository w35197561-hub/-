...
- **需修复** → 按下方问题分级表决定处理方，不得直接等用户

### 问题分级参考

| 档位              | 典型问题                                                  | 处理方              |
| ----------------- | --------------------------------------------------------- | ------------------- |
| 1 — lint 可修     | console.log、未用 import、命名格式                        | npm run lint 自动修 |
| 2 — reviewer 可修 | componentMap 字符串 key、直接赋值跳过 Command、6 处未同步 | reviewer 直接改代码 |
| 3 — 需用户决策    | 架构选择、业务逻辑歧义、不确定影响范围的改动              | 上报用户            |
