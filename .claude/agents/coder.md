...
1. 检查本次修法是否已在 `.claude/rules/coding.md` 中有对应规则：
   - **已有**：无需操作
   - **没有**：询问用户是否将该修法补入规则文件，由用户决定是否值得沉淀

2. 将本次验证情况追加到 `docs/flow-log.md` 的 Coder 段：
   - 自修轮次及每轮错误类型（无自修则写"无需自修"）
   - 用户干预次数
   - 写入后检查 flow-log 历史，若同类错误已出现 ≥ 2 次，主动提示用户考虑沉淀到 `coding.md`
